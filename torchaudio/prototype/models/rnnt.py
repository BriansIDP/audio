...
class RNNTBiasing(RNNT):
    ...
    def forward(
        ...
        if self.biasing and current_epoch >= self.tcpsche and tries != []:
            ...
            # hptr, tcpgen_dist = self.forward_tcpgen(targets, gt_mask, source_encodings)
            # if current_epoch < self.tcpsche + 1:
            #     p_gen_mask = 1 - (1-gt_mask).sum(dim=-1)
        ...
        if self.biasing and hptr is not None and tcpgen_dist is not None:
            ...
            # if current_epoch < self.tcpsche + 5:
                # p_gen = torch.nn.functional.threshold(p_gen, 0.2, 0.2)
                # p_gen = p_gen * 0.001
...
# def conformer_rnnt_biasing_clean(charlist=None, biasing=True) -> RNNT:
#     r"""Builds basic version of Conformer RNN-T model with TCPGen.
# 
#     Returns:
#         RNNT:
